...
import { Ai, AiTextGenerationInput, AiTextGenerationOutput, BaseAiTextGenerationModels, RoleScopedChatInput } from "@cloudflare/workers-types"
...
export const runWithTools = async (
  ...
  config: {
    ...
    trimFunction?: (tools: AiTextGenerationToolInputWithFunction[], ai: Ai, model: BaseAiTextGenerationModels, messages: RoleScopedChatInput[]) => Promise<AiTextGenerationToolInputWithFunction[]>
  } = {}
  ...
  const {
    ...
    verbose = false,
    trimFunction = async (tools: AiTextGenerationToolInputWithFunction[], ai: Ai, model: BaseAiTextGenerationModels, messages: RoleScopedChatInput[]) => tools as AiTextGenerationToolInputWithFunction[],
  ...
  async function runAndProcessToolCall({
    ...
    try {
      ...
      const chars = JSON.stringify(messages).length + JSON.stringify(initialtoolsWithoutFunctions).length
      ...
      const toolCallPromises = tool_calls.map(async (toolCall) => {
        ...
        const selectedTool = input.tools.find((tool) => tool.name === toolCallObjectJson.name)
      ...
      if (maxRecursiveToolRuns > 0 && tool_calls.length > 0) {
        ...
        Logger.info(`Number of characters for the final AI.run call: ${JSON.stringify(messages).length}`)
